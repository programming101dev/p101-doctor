...
1. `p101-wrapper-audit`, which checks whether source code bypasses available
   p101 wrappers;
2. `p101-module-map`, which checks module shape, public API surface, include
   relationships, and likely split/static-scope opportunities;
3. `p101-observe`, which captures resource events, call traces, resource
   findings, and correlated reports for an ordinary execution; and
4. `p101-error-path-walk`, which fails p101 calls one at a time and checks the
...
static wrapper audit.

## Usage

```sh
p101-doctor [-h] [-v] [-x] [-o <doctor-dir>] [-s <source-path>] [-n <count>] \
    [-A <p101-wrapper-audit>] [-M <p101-module-map>] \
...
The wrapper audit files are produced by `p101-wrapper-audit`; they are omitted
when `-x` is used. `module-map.md` is produced by `p101-module-map`; it is always
run because module/API shape is useful even when wrapper auditing is skipped.
...
`summary.md` starts with a quick grade for wrapper usage, module shape, runtime
resources, and error paths. `manifest.txt` records the exact tool paths, source
path, fault count, and target command used for the run.
